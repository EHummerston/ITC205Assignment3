...
public class testLoanDAO {
	...
	public void createLoan_NormalFlow_HelperMakeLoanMethodCalled () {
		...
		when(_helper.makeLoan(eq(_book), eq(_borrower), any(Date.class), any(Date.class))).thenReturn(loan);

		ILoan loan2 = _loanDao.createLoan(_borrower, _book);		
		verify(_helper).makeLoan(eq(_book), eq(_borrower), any(Date.class), any(Date.class));
