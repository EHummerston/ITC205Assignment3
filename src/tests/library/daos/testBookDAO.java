...
public class testBookDAO {
	...
	when(
		_helper.makeBook(eq(_author), eq(_title), eq(_callNumber),
			anyInt())).thenReturn(_book);
	ArgumentCaptor<Integer> argument = ArgumentCaptor
		.forClass(Integer.class);

	_bookDao.addBook(_author, _title, _callNumber);
	verify(_helper).makeBook(any(String.class), any(String.class),
		any(String.class), argument.capture());
