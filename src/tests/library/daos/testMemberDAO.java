...
public class testMemberDAO {
	...
	public void addMember_NormalFlow_MemberIdEqualsMapKey () {
		...
		when(_helper.makeMember(eq(_firstName), eq(_lastName), eq(_contactPhone), eq(_emailAddress), anyInt())).thenReturn(_member);
		ArgumentCaptor<Integer> argument = ArgumentCaptor.forClass(Integer.class);
		
		_memberDao.addMember(_firstName, _lastName, _contactPhone, _emailAddress);
		verify(_helper).makeMember(any(String.class), any(String.class), any(String.class), any(String.class), argument.capture());
